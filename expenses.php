...
if (isset($_GET['search'])) {
  ...
  $sql->selectall("trans where t_type=1 and stat=0 and name LIKE '%$search%'  ORDER BY id DESC limit 10 offset  $id");
  }else{
  ...
  for ($i=1; $i <=$page ; $i++) { 
   $id=$id+10;
    ...
    $sql->selectall("trans where t_type=2 and stat=0 ORDER BY id DESC limit 10 offset  $id ");
  }
...
?></center>
 ...
 $date=date_create($row['date2']);
    ...
    if (isset($_GET['search'])) {
              $search=$_GET['search'];
              $sql->selectall("trans  where t_type=2 name LIKE '$search%' ");
              $num=$sql->res->num_rows;
              $n=ceil($num/10+1);
    }else{
      $sql->check('trans',["t_type"=>2]);
       $n=ceil($sql->check/10+1) ;
            ...
            $page=$_GET['page']=1;
             ...
             if($page!=1){
              if (isset($_GET['search'])) {
                 echo '<li class="page-item "><a class="page-link" href="?exp=list&search='.$search.'&page='.$pp.'" tabindex="">Previous</a></li>';
              }else{
                 echo '<li class="page-item "><a class="page-link" href="?exp=list&page='.$pp.'" tabindex="">Previous</a></li>';
              }
              ...
              if ($get<0) {
               $get=1;
                    ...
                    if ($_GET['page']==$i) {
                     ...
                     if (isset($_GET['search'])) {
                  echo'<li class="page-item '.$a.'"><a class="page-link" href="?exp=list&search='.$search.'&page='.$i.'">'.$i.'</a></li>';
              }else{
                echo'<li class="page-item '.$a.'"><a class="page-link" href="?exp=list&page='.$i.'">'.$i.'</a></li>';              }
              ...
              for ($i=$_GET['page']; $i <$n ; $i++ ) { 
                  ...
                  if ($i<=$_GET['page']+5) {
                    ...
                    if (isset($_GET['search'])) {
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?exp=list&search='.$_GET['search'].'&page='.$i.'">'.$i.'</a></li>';
                    }else{
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?exp=list&page='.$i.'">'.$i.'</a></li>';
                    }
              ...
              if ($hi>1) {
                 if ($page!=$i) {
                  if (isset($_GET['search'])) {
                    echo '<a class="page-link" href="?exp=list&search='.$_GET['search'].'&page='.$page.'">Next</a>';
                  }else{
                    echo '<a class="page-link" href="?exp=list&page='.$page.'">Next</a>';
                  }
         ...
         <script type="text/javascript" class="init">
           $(document).ready(function() {
             $("#myInput").on("keyup", function() {
               ...
               $.post('inc/fun/expenses/select.php', {
                 search: value
               }, function(data) {
                 $(".kk").html(data)
               });
           ...
           <div class="modal-dialog" style="max-width: 70rem" role="document">
             <div class="modal-content">
               <div class="modal-header">
                 <h5 class="modal-title" id="exampleModalLabel1"> <?=$lang['Expenses']?></h5>
               ...
               <div class="modal-body">
                 <div class="row">
                   <form method="post" action="inc/fun/expenses/insert.php" enctype="multipart/form-data">
                     ...
                     <select style="display:none;" class="ex form-select ">

                       <?php

                                       $sql->selectall("account_no where account_c3 = 16 or account_c3 = 21 and stat=1");
                                       while($row=$sql->res->fetch_assoc())
         ...
         <script type="text/javascript">
           ...
           $(document).ready(function() {
             $(".ar").attr("href", "inc/des/lang.php?lang=ar&page='exp=list'");
             $(".en").attr("href", "inc/des/lang.php?lang=en&page='exp=list'");
           });
         </script>