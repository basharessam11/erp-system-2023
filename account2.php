...
if (isset($_GET['search'])) {
  ...
  $sql->selectall("account_c2 where c1=$get and name LIKE '$search%'   limit 10 offset  $id");
  }else{
  ...
  for ($i=1; $i <=$page ; $i++) { 
   $id=$id+10;
    ...
    $sql->selectall("account_c2 where c1=$get ORDER BY name limit 10 offset  $id ");
  }
...
$sql->select1("account_c1"," where id=$get");
while ($row1 = $sql->res1->fetch_assoc()) {
  echo $row["name"]  ." < " . $row1['account_name'];
}
    ...
    ?></center>
                       ...
                       <td style="padding: .625rem 0.25rem;">
                         <center>
                           ...
                           <button type="button" class="id btn btn-primary" data-bs-toggle="modal" data-bs-target="#basicModal" name="<?=$row["name"]?>" account1="<?=$row["c1"]?>" besho="<?=$row["id"]?>">
                             <i class='bx bxs-edit'></i>

                           </button>

                           <a href="?account_c3=list&id=<?=$row["id"]?>">
    ...
    if (isset($_GET['search'])) {
              $search=$_GET['search'];
              $sql->selectall("account_c2  where c1=$get and name LIKE '$search%' ");
              $num=$sql->res->num_rows;
              $n=ceil($num/10+1);
    }else{
      $sql->check('account_c2',["c1"=>$get]);
       $n=ceil($sql->check/10+1) ;
            ...
            $page=$_GET['page']=1;
             ...
             if($page!=1){
              if (isset($_GET['search'])) {
                 echo '<li class="page-item "><a class="page-link" href="?account_c2=list&search='.$search.'&page='.$pp.'" tabindex="">Previous</a></li>';
              }else{
                 echo '<li class="page-item "><a class="page-link" href="?account_c2=list&page='.$pp.'" tabindex="">Previous</a></li>';
              }
              ...
              if ($get<0) {
               $get=1;
                    ...
                    if ($_GET['page']==$i) {
                     ...
                     if (isset($_GET['search'])) {
                  echo'<li class="page-item '.$a.'"><a class="page-link" href="?account_c2=list&search='.$search.'&page='.$i.'">'.$i.'</a></li>';
              }else{
                echo'<li class="page-item '.$a.'"><a class="page-link" href="?account_c2=list&page='.$i.'">'.$i.'</a></li>';              }
              ...
              for ($i=$_GET['page']; $i <$n ; $i++ ) { 
                  ...
                  if ($i<=$_GET['page']+5) {
                    ...
                    if (isset($_GET['search'])) {
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?account_c2=list&search='.$_GET['search'].'&page='.$i.'">'.$i.'</a></li>';
                    }else{
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?account_c2=list&page='.$i.'">'.$i.'</a></li>';
                    }
              ...
              if ($hi>1) {
                 if ($page!=$i) {
                  if (isset($_GET['search'])) {
                    echo '<a class="page-link" href="?account_c2=list&search='.$_GET['search'].'&page='.$page.'">Next</a>';
                  }else{
                    echo '<a class="page-link" href="?account_c2=list&page='.$page.'">Next</a>';
                  }
         ...
         <script type="text/javascript" class="init">
           $(document).ready(function() {
             $("#myInput").on("keyup", function() {
               ...
               $.post('inc/fun/account_c2/select.php', {
                 search: value
               }, function(data) {
                 $(".kk").html(data)
               });
           ...
           <div class="modal-dialog" style="max-width: 70rem" role="document">
             <div class="modal-content">
               ...
               <div class="modal-body">
                 <div class="row">
                   <form method="post" action="inc/fun/account_c2/update.php" enctype="multipart/form-data">
         ...
         <div class="modal fade" id="basicModal1" tabindex="-1" aria-hidden="true">
           <div class="modal-dialog" style="max-width: 70rem" role="document">
             <div class="modal-content">
               ...
               <div class="modal-body">
                 <div class="row">
                   <form method="post" action="inc/fun/account_c2/insert.php" enctype="multipart/form-data">
                     <div class="col mb-3">
                       <label for="nameBasic" class="form-label"><?=$lang['NAME']?> </label>
         ...
         <div class="modal fade" id="basicModal2" tabindex="-1" aria-hidden="true">
           <div class="modal-dialog" role="document">
             <div class="modal-content">
               ...
               <div class="modal-body">
                 <div class="row">
                   <form method="post" action="inc/fun/account_c2/delete.php">
