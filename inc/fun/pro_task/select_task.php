...
<table width="100%"  class="datatables-basic table table-bordered ">
  ...
  $sql->selectall("pro_task where user_id=$user_id and name LIKE '$search%' ORDER BY id DESC  limit 10 offset  $id");
...
<?php
        ...
        $sql->selectall("pro_task  where user_id=$user_id and  name LIKE '$search%' ");
...
<?php
  ...
  $sql->selectall("pro_task  where user_id=$user_id  ORDER BY id DESC limit 10   offset  $id ");
...
<?php
       ...
       $sql->check('pro_task',["user_id"=>"$user_id"]);
