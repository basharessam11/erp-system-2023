           ...
           <div class="row g-4 mb-4">
             ...
             <div class="card">
               <div class="card-header border-bottom">
                 <h5 class="card-title" style="float:left;"><?=$lang['gl']?></h5>
...
if (isset($_GET['search'])) {
  ...
  $sql->selectall("trans where   stat =0 and name LIKE '%$search%'  ORDER BY id DESC limit 10 offset  $id");
  }else{
  ...
  for ($i=1; $i <=$page ; $i++) { 
   $id=$id+10;
    ...
    $sql->selectall("trans where  stat =0 ORDER BY id DESC limit 10 offset  $id ");
  }
...
?></center>
 ...
 $date=date_create($row['date2']);
    ...
    if (isset($_GET['search'])) {
              $search=$_GET['search'];
              $sql->selectall("trans  where t_type=1 and stat =0  name LIKE '$search%' ");
              $num=$sql->res->num_rows;
              $n=ceil($num/10+1);
    }else{
      $sql->check('trans',["t_type"=>1,"stat"=>0]);
       $n=ceil($sql->check/10+1) ;
            ...
            $page=$_GET['page']=1;
             ...
             if($page!=1){
              if (isset($_GET['search'])) {
                 echo '<li class="page-item "><a class="page-link" href="?gl=list&search='.$search.'&page='.$pp.'" tabindex="">Previous</a></li>';
              }else{
                 echo '<li class="page-item "><a class="page-link" href="?gl=list&page='.$pp.'" tabindex="">Previous</a></li>';
              }
              ...
              if ($get<0) {
               $get=1;
                    ...
                    if ($_GET['page']==$i) {
                     ...
                     if (isset($_GET['search'])) {
                  echo'<li class="page-item '.$a.'"><a class="page-link" href="?gl=list&search='.$search.'&page='.$i.'">'.$i.'</a></li>';
              }else{
                echo'<li class="page-item '.$a.'"><a class="page-link" href="?gl=list&page='.$i.'">'.$i.'</a></li>';              }
              ...
              for ($i=$_GET['page']; $i <$n ; $i++ ) { 
                  ...
                  if ($i<=$_GET['page']+5) {
                    ...
                    if (isset($_GET['search'])) {
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?gl=list&search='.$_GET['search'].'&page='.$i.'">'.$i.'</a></li>';
                    }else{
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?gl=list&page='.$i.'">'.$i.'</a></li>';
                    }
              ...
              if ($hi>1) {
                 if ($page!=$i) {
                  if (isset($_GET['search'])) {
                    echo '<a class="page-link" href="?gl=list&search='.$_GET['search'].'&page='.$page.'">Next</a>';
                  }else{
                    echo '<a class="page-link" href="?gl=list&page='.$page.'">Next</a>';
                  }
         ...
         <script type="text/javascript">
           ...
           $(document).ready(function() {
             $(".ar").attr("href", "inc/des/lang.php?lang=ar&page='gl=list'");
             $(".en").attr("href", "inc/des/lang.php?lang=en&page='gl=list'");
           });
         </script>