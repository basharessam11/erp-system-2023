...
if ($stat==1) {
  echo '<div class="alert alert-warning">Waiting for manager approval<div>';
    ...
    if (isset($_GET['search'])) {
              $search=$_GET['search'];
              $sql->selectall("emp_leave  where stat=2  name LIKE '$search%' ");
      ...
      $sql->check('emp_leave',["stat"=>2]);
       $n=ceil($sql->check/10+1) ;
            ...
            $page=$_GET['page']=1;
             ...
             if($page!=1){
              if (isset($_GET['search'])) {
                 echo '<li class="page-item "><a class="page-link" href="?hr=list&search='.$search.'&page='.$pp.'" tabindex="">Previous</a></li>';
              }else{
              ...
              for ($i=$_GET['page']; $i <$n ; $i++ ) { 
                  ...
                  if ($i<=$_GET['page']+5) {
                    ...
                    if (isset($_GET['search'])) {
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?hr=list&search='.$_GET['search'].'&page='.$i.'">'.$i.'</a></li>';
                    }else{
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?hr=list&page='.$i.'">'.$i.'</a></li>';
                    }
              ...
              if ($hi>1) {
                 if ($page!=$i) {
                  if (isset($_GET['search'])) {
                    echo '<a class="page-link" href="?hr=list&search='.$_GET['search'].'&page='.$page.'">Next</a>';
                  }else{
         ...
         <script type="text/javascript" class="init">
           $(document).ready(function() {
             $("#myInput").on("keyup", function() {
               ...
               $.post('inc/fun/leave/select_hr.php', {
                 search: value
               }, function(data) {
                 $(".kk").html(data)
               });
         ...
         <script>
           ...
           $(document).ready(function() {
             $(".ar").attr("href", "inc/des/lang.php?lang=ar&page='hr=list'");
             $(".en").attr("href", "inc/des/lang.php?lang=en&page='hr=list'");
           });
         </script>