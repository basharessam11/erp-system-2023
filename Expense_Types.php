           ...
           <div class="row g-4 mb-4">
             ...
             <div class="card">
               <div class="card-header border-bottom">
                 <h5 class="card-title" style="float:left;"> <?=$lang['Expense_Types']?> </h5>
...
if (isset($_GET['search'])) {
  ...
  $sql->selectall("Expense_Types where  name LIKE '$search%'   limit 10 offset  $id");
  }else{
  ...
  for ($i=1; $i <=$page ; $i++) { 
   $id=$id+10;
    ...
    $sql->selectall("Expense_Types ORDER BY name limit 10 offset  $id ");
  }
                     ...
                     <tr>
                       ...
                       <td style="padding: .625rem 0.25rem;">
                         <center>
                           ...
                           <button type="button" class="id btn btn-primary" data-bs-toggle="modal" data-bs-target="#basicModal" name="<?=$row["name"]?>" c3="<?=$row["c3"]?>" besho="<?=$row["id"]?>">
    ...
    if (isset($_GET['search'])) {
              $search=$_GET['search'];
              $sql->selectall("Expense_Types  where  name LIKE '$search%' ");
              $num=$sql->res->num_rows;
              $n=ceil($num/10+1);
    }else{
      $sql->check('Expense_Types',["1"=>1]);
       $n=ceil($sql->check/10+1) ;
            ...
            $page=$_GET['page']=1;
             ...
             if($page!=1){
              if (isset($_GET['search'])) {
                 echo '<li class="page-item "><a class="page-link" href="?Expense_Types=list&search='.$search.'&page='.$pp.'" tabindex="">Previous</a></li>';
              }else{
                 echo '<li class="page-item "><a class="page-link" href="?Expense_Types=list&page='.$pp.'" tabindex="">Previous</a></li>';
              }
              ...
              if ($get<0) {
               $get=1;
                    ...
                    if ($_GET['page']==$i) {
                     ...
                     if (isset($_GET['search'])) {
                  echo'<li class="page-item '.$a.'"><a class="page-link" href="?Expense_Types=list&search='.$search.'&page='.$i.'">'.$i.'</a></li>';
              }else{
                echo'<li class="page-item '.$a.'"><a class="page-link" href="?Expense_Types=list&page='.$i.'">'.$i.'</a></li>';              }
              ...
              for ($i=$_GET['page']; $i <$n ; $i++ ) { 
                  ...
                  if ($i<=$_GET['page']+5) {
                    ...
                    if (isset($_GET['search'])) {
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?Expense_Types=list&search='.$_GET['search'].'&page='.$i.'">'.$i.'</a></li>';
                    }else{
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?Expense_Types=list&page='.$i.'">'.$i.'</a></li>';
                    }
              ...
              if ($hi>1) {
                 if ($page!=$i) {
                  if (isset($_GET['search'])) {
                    echo '<a class="page-link" href="?Expense_Types=list&search='.$_GET['search'].'&page='.$page.'">Next</a>';
                  }else{
                    echo '<a class="page-link" href="?Expense_Types=list&page='.$page.'">Next</a>';
                  }
         ...
         <script type="text/javascript" class="init">
           $(document).ready(function() {
             $("#myInput").on("keyup", function() {
               ...
               $.post('inc/fun/Expense_Types/select.php', {
                 search: value
               }, function(data) {
                 $(".kk").html(data)
               });
           ...
           <div class="modal-dialog" style="max-width: 70rem" role="document">
             <div class="modal-content">
               <div class="modal-header">
                 <h5 class="modal-title" id="exampleModalLabel1">edit Expense_Types</h5>
                 <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
               </div>
               <div class="modal-body">
                 <div class="row">
                   <form method="post" action="inc/fun/Expense_Types/update.php" enctype="multipart/form-data">

                     <div class="col mb-3">
                       <label for="nameBasic" class="form-label"><?=$lang['NAME']?>:</label>
                       <input type="text" id="nameBasic" class="name form-control" name="name" required="" placeholder="Name">

                     </div>

                     <div class="col mb-3">

                       <label><span class="h6">Account</span></label>
                       <select name="account_c3" class="c3 form-select ">
         ...
         <div class="modal fade" id="basicModal1" tabindex="-1" aria-hidden="true">
           <div class="modal-dialog" style="max-width: 70rem" role="document">
             <div class="modal-content">
               <div class="modal-header">
                 <h5 class="modal-title" id="exampleModalLabel1">Add New Expense_Types</h5>
                 <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
               </div>
               <div class="modal-body">
                 <div class="row">
                   <form method="post" action="inc/fun/Expense_Types/insert.php" enctype="multipart/form-data">
                     <div class="col mb-3">
                       <label for="nameBasic" class="form-label"><?=$lang['NAME']?> :</label>
                     ...
                     <div class="col mb-3">
                       ...
                       <label><span class="h6">Account</span></label>
                       <select name="account_c3" class=" form-select ">
         ...
         <div class="modal fade" id="basicModal2" tabindex="-1" aria-hidden="true">
           <div class="modal-dialog" role="document">
             <div class="modal-content">
               ...
               <div class="modal-body">
                 <div class="row">
                   <form method="post" action="inc/fun/Expense_Types/delete.php">
         ...
         <script>
           function data() {
             var Expense_Types = get_filter('mat');
