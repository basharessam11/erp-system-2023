...
<table width="100%"  class="datatables-basic table table-bordered ">
      <thead>
        <tr>
          ...
          <th style="padding: .625rem 0.25rem;"><center>الاسم</center> </th>
          <th style="padding: .625rem 0.25rem;"><center>المبلغ</center> </th>
  ...
  $sql->selectall("pro_a where  name LIKE '$search%'  ORDER BY id DESC  limit 10 offset  $id");
  ...
  echo '<div class="alert alert-danger"><center><b> not found</b> </center></div>';
     ...
     while ($row = $sql->res->fetch_assoc()) {
      ...
      <td style="padding: .625rem 0.25rem;"><center ><?=$row["amount"]?></center></td>


                  <td style="padding: .625rem 0.25rem;"><center>
          
         <button type="button" class="id btn btn-primary" data-bs-toggle="modal" data-bs-target="#basicModal" name="<?=$row["name"]?>" amount="<?=$row["amount"]?>"   besho="<?=$row["id"]?>">
...
<?php
  ...
  $sql->selectall("pro_a  ORDER BY id DESC limit 10   offset  $id ");
