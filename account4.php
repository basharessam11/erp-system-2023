 ...
 <center> رقم الحساب ﻣﻮﺟﻮﺩ ﺑﺎﻟﻔﻌﻞ </center>
</div>';
          }
...
<center> ﺗﻢ ﺗﻐﻴﻴﺮ اﺳﻢ اﻟﻤﻮﻗﻊ ﺑﻨﺠﺎﺡ</center>
          ...
          ?>
               ...
               <div class="row">
                 <div style="dis" class="col-sm-12 col-md-6">
...
if (isset($_GET['search'])) {
  ...
  $sql->selectall("account_no where account_c3=$get and account_name LIKE '$search%'   limit 10 offset  $id");
  }else{
  ...
  for ($i=1; $i <=$page ; $i++) { 
   $id=$id+10;
    ...
    $sql->selectall("account_no where account_c3=$get ORDER BY account_name limit 10 offset  $id ");
  }
  
  
      $sql->select1("account_c3"," where id=$get");
...
$sql->select1("account_c1"," where id=$c1");
while ($row1 = $sql->res1->fetch_assoc()) {
  echo   $row["account_name"] ."<".$c3 ." < " .$c2 ." < ". $row1['account_name'];;
}
    ...
    ?></center>
                       ...
                       <td style="padding: .625rem 0.25rem;">
                         <center>
                           ...
                           <button type="button" class="id btn btn-primary" data-bs-toggle="modal" data-bs-target="#basicModal" name="<?=$row["account_name"]?>" account1="<?=$row["account_number"]?>" balance="<?=$row["balance"]?>" dr="<?=$row["dr"]?>" currency="<?=$row["currency"]?>" besho="<?=$row["id"]?>">
    ...
    if (isset($_GET['search'])) {
              $search=$_GET['search'];
              $sql->selectall("account_no  where account_c3=$get and name LIKE '$search%' ");
              $num=$sql->res->num_rows;
              $n=ceil($num/10+1);
    }else{
      $sql->check('account_no',["account_c3"=>$get]);
       $n=ceil($sql->check/10+1) ;
            ...
            $page=$_GET['page']=1;
             ...
             if($page!=1){
              if (isset($_GET['search'])) {
                 echo '<li class="page-item "><a class="page-link" href="?account_no=list&search='.$search.'&page='.$pp.'" tabindex="">Previous</a></li>';
              }else{
                 echo '<li class="page-item "><a class="page-link" href="?account_no=list&page='.$pp.'" tabindex="">Previous</a></li>';
              }
              ...
              if ($get<0) {
               $get=1;
                    ...
                    if ($_GET['page']==$i) {
                     ...
                     if (isset($_GET['search'])) {
                  echo'<li class="page-item '.$a.'"><a class="page-link" href="?account_no=list&search='.$search.'&page='.$i.'">'.$i.'</a></li>';
              }else{
                echo'<li class="page-item '.$a.'"><a class="page-link" href="?account_no=list&page='.$i.'">'.$i.'</a></li>';              }
              ...
              for ($i=$_GET['page']; $i <$n ; $i++ ) { 
                  ...
                  if ($i<=$_GET['page']+5) {
                    ...
                    if (isset($_GET['search'])) {
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?account_no=list&search='.$_GET['search'].'&page='.$i.'">'.$i.'</a></li>';
                    }else{
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?account_no=list&page='.$i.'">'.$i.'</a></li>';
                    }
              ...
              if ($hi>1) {
                 if ($page!=$i) {
                  if (isset($_GET['search'])) {
                    echo '<a class="page-link" href="?account_no=list&search='.$_GET['search'].'&page='.$page.'">Next</a>';
                  }else{
                    echo '<a class="page-link" href="?account_no=list&page='.$page.'">Next</a>';
                  }
         ...
         <div class="modal fade" id="basicModal" tabindex="-1" aria-hidden="true">
           <div class="modal-dialog" style="max-width: 70rem" role="document">
             <div class="modal-content">
                   ...
                   <form method="post" action="inc/fun/account_no/update.php" enctype="multipart/form-data">
                     <div class="form-group">
                       <div class="row">
                         ...
                         <div class="col-md-6">
                           ...
                           <input type="text" class="num form-control" id="number" name="number" placeholder="<?php echo $lang['Account_Number'];?>">
                         ...
                         <div class="col-md-6">
                           <br>
                           <label><?php echo $lang['Currencies'];?></label>

                           <select class="car form-select" name="currency">
                             <?php
                     ...
                     <script>
                       $(".id").click(function() {
                         ...
                         var account1 = $(this).attr('account1');
               ...
               <input value="<?=$_GET['id']?>" type="hidden" name="a">
                     ...
                     <div class="form-group">
                       <div class="row">
                         ...
                         <div class="col-md-6">
                           <br>
                           <label><?php echo $lang['Account_Number'];?></label>
                           <input type="text" class="form-control" id="number" name="number" placeholder="<?php echo $lang['Account_Number'];?>">
