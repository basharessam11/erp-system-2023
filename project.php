           ...
           <div class="row g-4 mb-4">
             ...
             <div class="card">
               <div class="card-header border-bottom">
                 <h5 class="card-title" style="float:left;"> <?=$lang['Projects']?> </h5>
                 ...
                 <table width="10%" class="datatables-basic  table table-bordered">
                   <thead>
                     <tr>
                       ...
                       <th style="padding: .625rem 0.25rem;">
                         <center><?=$lang['Start_Date']?></center< /center>
                       </th>
                       <th style="padding: .625rem 0.25rem;">
                         <center><?=$lang['end_Date']?></center< /center>
                       </th>
                       <th style="padding: .625rem 0.25rem;">
                         <center><?=$lang['Project_Manager']?></center< /center>
                       </th>

                       <th style="padding: .625rem 0.25rem;">
                         <center><?=$lang['category']?></center< /center>
                       </th>
...
if (isset($_GET['search'])) {
  ...
  $sql->selectall("project where user_id=$user_id and  name LIKE '$search%'   limit 10 offset  $id");
  }else{
  ...
  for ($i=1; $i <=$page ; $i++) { 
   $id=$id+10;
    ...
    $sql->selectall("project  where user_id=$user_id ORDER BY id desc limit 10 offset  $id ");
  }
...
echo$row1['name'];
    ...
    if (isset($_GET['search'])) {
              $search=$_GET['search'];
              $sql->selectall("project  where  user_id=$user_id and name LIKE '$search%' ");
              $num=$sql->res->num_rows;
              $n=ceil($num/10+1);
    }else{
      $sql->check('project',["user_id"=>$user_id]);
       $n=ceil($sql->check/10+1) ;
            ...
            $page=$_GET['page']=1;
             ...
             if($page!=1){
              if (isset($_GET['search'])) {
                 echo '<li class="page-item "><a class="page-link" href="?project=list&search='.$search.'&page='.$pp.'" tabindex="">Previous</a></li>';
              }else{
                 echo '<li class="page-item "><a class="page-link" href="?project=list&page='.$pp.'" tabindex="">Previous</a></li>';
              }
              ...
              if ($get<0) {
               $get=1;
                    ...
                    if ($_GET['page']==$i) {
                     ...
                     if (isset($_GET['search'])) {
                  echo'<li class="page-item '.$a.'"><a class="page-link" href="?project=list&search='.$search.'&page='.$i.'">'.$i.'</a></li>';
              }else{
                echo'<li class="page-item '.$a.'"><a class="page-link" href="?project=list&page='.$i.'">'.$i.'</a></li>';              }
              ...
              for ($i=$_GET['page']; $i <$n ; $i++ ) { 
                  ...
                  if ($i<=$_GET['page']+5) {
                    ...
                    if (isset($_GET['search'])) {
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?project=list&search='.$_GET['search'].'&page='.$i.'">'.$i.'</a></li>';
                    }else{
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?project=list&page='.$i.'">'.$i.'</a></li>';
                    }
              ...
              if ($hi>1) {
                 if ($page!=$i) {
                  if (isset($_GET['search'])) {
                    echo '<a class="page-link" href="?project=list&search='.$_GET['search'].'&page='.$page.'">Next</a>';
                  }else{
                    echo '<a class="page-link" href="?project=list&page='.$page.'">Next</a>';
                  }
         ...
         <script type="text/javascript" class="init">
           $(document).ready(function() {
             $("#myInput").on("keyup", function() {
               ...
               $.post('inc/fun/project/select1.php', {
                 search: value,
                 user_id: < ? = $user_id ? >
               }, function(data) {
                 $(".kk").html(data)
           ...
           <div class="modal-dialog" role="document">
             <div class="modal-content">
               ...
               <div class="modal-body">
                 <div class="row">
                   <form method="post" action="inc/fun/project/delete.php">
         ...
         <script>
           ...
           $(document).ready(function() {
             $(".ar").attr("href", "inc/des/lang.php?lang=ar&page='project=list'");
             $(".en").attr("href", "inc/des/lang.php?lang=en&page='project=list'");
           });
         </script>