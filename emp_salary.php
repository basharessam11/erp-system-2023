           ...
           <div class="row g-4 mb-4">
             ...
             <div class="card">
               <div class="card-header border-bottom">
                 <h5 class="card-title" style="float:left;">emp salary List </h5>
               ...
               <div class="row">
                 <div class="col-sm-12 col-md-6">
                   <div class="dataTables_length" id="DataTables_Table_1_length">
                     ...
                     <label>Search: </label>
                     <br>
                     <select style="width: 50%" id="myInput" class="depart select form-select js-example-basic-single">
...
$array=[];
 $sql->select1("salary_slip2","where emp_id=$emp_id");
  ...
  $imp=implode(" or id = ", $array)??0;
 }



 $sql->select1("emp_slip","where id=$imp");
     ...
     while ($row1 = $sql->res1->fetch_assoc()) {
                              ...
                              ?>
                       <option value="<?=$row1['id']?>"><?=$row1['date']?></option>
                       <?php
...
echo date_format($date,"y-M ");
 ...
 while ($row=$sql->res_sum->fetch_assoc()) {
  ...
  ?></center>
    ...
    if (isset($_GET['search'])) {
              $search=$_GET['search'];
              $sql->selectall("emp_slip  where  id =$search ");
      ...
      $sql->check('emp_slip ',["id"=>"$imp"]);
       $n=ceil($sql->check/10+1) ;
            ...
            $page=$_GET['page']=1;
             ...
             if($page!=1){
              if (isset($_GET['search'])) {
                 echo '<li class="page-item "><a class="page-link" href="?emp_salary=list&id='.$emp_id.'&search='.$search.'&page='.$pp.'" tabindex="">Previous</a></li>';
              }else{
              ...
              for ($i=$_GET['page']; $i <$n ; $i++ ) { 
                  ...
                  if ($i<=$_GET['page']+5) {
                    ...
                    if (isset($_GET['search'])) {
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?emp_salary=list&id='.$emp_id.'&search='.$_GET['search'].'&page='.$i.'">'.$i.'</a></li>';
                    }else{
                      echo'<li class="page-item '.$a.'"><a class="page-link" href="?emp_salary=list&id='.$emp_id.'&page='.$i.'">'.$i.'</a></li>';
                    }
              ...
              if ($hi>1) {
                 if ($page!=$i) {
                  if (isset($_GET['search'])) {
                    echo '<a class="page-link" href="?emp_salary=list&id='.$emp_id.'&search='.$_GET['search'].'&page='.$page.'">Next</a>';
                  }else{
